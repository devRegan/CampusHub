...
-- Add index for app_settings id (primary key exists) and for policies referencing columns
CREATE INDEX IF NOT EXISTS idx_profiles_id ON public.profiles(id);
CREATE INDEX IF NOT EXISTS idx_posts_user_id_partial ON public.posts(user_id);
...
-- =====================================================
-- RLS: app_settings policies (safe defaults)
-- =====================================================

-- App settings: allow authenticated users to read, but only admins to modify.
...
BEGIN
  IF NOT EXISTS (
    ...
    JOIN pg_class c ON p.polrelid = c.oid
    JOIN pg_namespace n ON c.relnamespace = n.oid
    WHERE p.polname = 'Authenticated can read app_settings'
...
BEGIN
  IF NOT EXISTS (
    ...
    JOIN pg_class c ON p.polrelid = c.oid
    JOIN pg_namespace n ON c.relnamespace = n.oid
    WHERE p.polname = 'Admins can insert app_settings'
      AND c.relname = 'app_settings'
      AND n.nspname = 'public'
  ) THEN
...
BEGIN
  IF NOT EXISTS (
    ...
    JOIN pg_class c ON p.polrelid = c.oid
    JOIN pg_namespace n ON c.relnamespace = n.oid
    WHERE p.polname = 'Admins can update app_settings'
      AND c.relname = 'app_settings'
      AND n.nspname = 'public'
  ) THEN
...
BEGIN
  IF NOT EXISTS (
    ...
    JOIN pg_class c ON p.polrelid = c.oid
    JOIN pg_namespace n ON c.relnamespace = n.oid
    WHERE p.polname = 'Admins can delete app_settings'
      AND c.relname = 'app_settings'
      AND n.nspname = 'public'
  ) THEN
